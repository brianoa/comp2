 ...
 */
class DanaDanaCollection extends \yii\db\ActiveRecord
{
    ...
    public static function getMpesaCounts($type){
        ...
        case 'today':
            ...
            $sum = DanaDanaCollection::getTotalMpesaInRange($midnight,$today)['total_mpesa'];
            break;
        case 'yesterday':
            $yestFloor = date( 'Y-m-d 00:00:00',strtotime('-1 day', time()));
            $yestCeil = date( 'Y-m-d 23:59:59',strtotime('-1 day', time()));
            $sum = DanaDanaCollection::getTotalMpesaInRange($yestFloor, $yestCeil)['total_mpesa'];
            break;
        case 'last_7_days':
           // $_7daysFloor = date( 'Y-m-d 00:00:00',strtotime('-7 day', time())); //Change to check from monday to today
            ...
            $sum = DanaDanaCollection::getTotalMpesaInRange($_lastMonday, $today)['total_mpesa'];
            break;
        case 'currentmonth':
            $cFloor = date( 'Y-m-1 00:00:00');
            $sum = DanaDanaCollection::getTotalMpesaInRange($cFloor, $today)['total_mpesa'];
            break;
        case 'lastweek':
            ...
            $sum = DanaDanaCollection::getTotalMpesaInRange($floorDate, $ceilDate)['total_mpesa'];
            break;
        case 'lastmonth':
            $lFloor = date( 'Y-m-1 00:00:00',strtotime('-1 month', time()));
            $lCeil = date('Y-m-d 23:59:59', strtotime('last day of previous month'));
            $sum = DanaDanaCollection::getTotalMpesaInRange($lFloor, $lCeil)['total_mpesa'];
            break;
        default :   
            $sum = DanaDanaCollection::getTotalRevenue()['total_mpesa'];
        endswitch;
        return $sum;    
    }
    public static function getMpesaCountsPerStation($type,$station_id){
        ...
        case 'today':
            $midnight = date('Y-m-d 00:00:00');
            $sum = DanaDanaCollection::getTotalMpesaInRangePerStation($midnight,$today,$station_id)['total_mpesa'];
            break;
        case 'yesterday':
            $yestFloor = date( 'Y-m-d 00:00:00',strtotime('-1 day', time()));
            $yestCeil = date( 'Y-m-d 23:59:59',strtotime('-1 day', time()));
            $sum = DanaDanaCollection::getTotalMpesaInRangePerStation($yestFloor, $yestCeil,$station_id)['total_mpesa'];
            break;
        case 'last_7_days':
           // $_7daysFloor = date( 'Y-m-d 00:00:00',strtotime('-7 day', time())); //Change to check from monday to today
            $_lastMonday = date('Y-m-d 00:00:00',strtotime('Monday this week'));
            $sum = DanaDanaCollection::getTotalMpesaInRangePerStation($_lastMonday, $today,$station_id)['total_mpesa'];
            break;
        case 'currentmonth':
            $cFloor = date( 'Y-m-1 00:00:00');
            $sum = DanaDanaCollection::getTotalMpesaInRangePerStation($cFloor, $today,$station_id)['total_mpesa'];
            break;
        case 'lastweek':
            $floorDate = date("Y-m-d 00:00:00", strtotime("Monday last week"));
            $ceilDate = date("Y-m-d 23:59:59", strtotime("Sunday last week"));
            $sum = DanaDanaCollection::getTotalMpesaInRangePerStation($floorDate, $ceilDate,$station_id)['total_mpesa'];
            break;
        case 'lastmonth':
            //$lFloor = date( 'Y-m-1 00:00:00',strtotime('-1 month', time()));
            $lFloor =date('Y-m-d 00:00:00', strtotime('first day of last month'));
            $lCeil = date('Y-m-d 23:59:59', strtotime('last day of previous month'));
            $sum = DanaDanaCollection::getTotalMpesaInRangePerStation($lFloor, $lCeil,$station_id)['total_mpesa'];
            break;
        default :   
            $sum1 = DanaDanaCollection::getTotalRevenuePerStation($station_id)['total_mpesa'];
       ...
       // ini_set('auto_detect_line_endings', TRUE);
        ...
        while (($fileop = fgetcsv($handle, 1000, ",")) !== false) 
        {
            if($row >= 6){
                ...
                $mod = DanaDanaCollection::find()->where("TransID = '$col1'")->one();
                if(!$mod){
            ...
            for($i=0; $i<count($stations); $i++)
            {
                ...
                try
                {
                    ...
                    if($report==NULL)
                    {
                        ...
                        $model->total_revenue=DanaDanaCollection::getTotalMpesaPerStation($revenue_date,$row->id)['total_mpesa'];
                    ...
                    else
                    {
                        ...
                        $report->total_revenue=DanaDanaCollection::getTotalMpesaPerStation($revenue_date,$row->id)['total_mpesa'];
    ...
    {
        $total=DanaDanaCollection::countAssignedCode($created_at)['total'];
        $unassigned=DanaDanaCollection::countUnAssignedCode($created_at)['total'];
        $data=DanaDanaCollection::getAssignedPerStation($created_at);
        for($i=0; $i<count($data); $i++)
        {
            ...
            if($stop > 0)
            {
                ...
                DanaDanaCollection::assignCode($row['station_id'],$created_at,$stop,$station_code);
            }
        }
    }
    ...
    {
        $data=DanaDanaCollection::find()->where("station_id IS NULL")->all();
        foreach($data as $row)
        {
    ...
    {
        $data=DanaDanaCollection::find()->where("created_at < '$created_at'")->limit($limit)->all();
        $rows="";
        $length=count($data);
