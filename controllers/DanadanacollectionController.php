...
use app\models\DanaDanaCollectionSearch;
use Yii;
 ...
 */
class DanadanacollectionController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    {
       try {
            $searchModel  = new DanaDanaCollectionSearch();
            $model        = new DanaDanaCollection();
        ...
        $act -> desc = "Danadana mpesapayment report";
        $act ->setLog();
    ...
    /**
     * Finds the DanaDanaCollection model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param string $id
     * @return DanaDanaCollection the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = DanaDanaCollection::findOne($id)) !== null) {
            return $model;
        }
    ...
    {
        Myhelper::checkRemoteAddress();
        $dups=DanaDanaCollection::getDuplicates();
        for($i=0;$i < count($dups); $i++)
        {
            $row=$dups[$i];
            DanaDanaCollection::removeDups($row['TransID'],$row['total']-1);
        }
    }
...
//			$file    = "confirmed_payment.csv";
			...
			if ( ( $handle = fopen( $file, "r" ) ) !== false ) {
				while ( ( $data = fgetcsv( $handle, 2000, "," ) ) !== false ) {
                    ...
                    && !empty($balance)) {
							$check_if_exists = DanaDanaCollection::find()->where( [ 'TransID' => $transaction_number ] )->one();
...
//			$file    = "confirmed_payment.csv";
    ...
    {
        $data=DanaDanaCollection::find()->where("operator='airtel'")->andWhere("created_at > '2022-07-02'")->all();
        foreach($data as $row)
        {
            try{
    ...
    {
        $data=DanaDanaCollection::find()->where('created_at > "2022-11-01"')->all();
        foreach($data as $row)
        {
    ...
    public function actionArchive($limit)
    {
        ...
        DanaDanaCollection::archive($created_at,$limit);
    }
    public function actionDownload() {
		if ( isset( $_POST['submit'] ) )
        {
            ...
            {
                DanaDanaCollection::getData($from,$to,$reference);
            }
            if(!empty($from) && !empty($to) && !empty($reference))
            {
                DanaDanaCollection::getData($from,$to,$reference);
